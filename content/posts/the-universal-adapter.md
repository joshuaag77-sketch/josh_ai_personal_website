...
summary: "MHS doesn't make machines smart. It gives an AI a common software interface and an instruction sheet for machines that are already digitally controllable. Then the AI can coordinate them at a higher level than engineers programming every workflow by hand."
kicker: "Field Notes"
...
The thing that made the difference is called the Model Hardware Standard. Anthropic previewed it on August 27. I think it's quietly one of the more important AI announcements this year, and I want to explain it in plain terms, because the idea is simple once you see the chain.
...
An MHS driver does two things.

It translates a machine's existing controls into a common interface. Underneath, that's a small set of commands, read this value, set that one. A CNC mill and an inspection camera look the same to the AI at that level.
...
Anthropic wants MHS to do something similar for physical equipment. Same idea, one floor down.

## Why this is different from normal automation

I spent five years around industrial equipment at Enbridge, so my first reaction was, we already automate machines. Plants run on PLCs and OPC UA. Labs run on SiLA. Robots run on ROS. All of that works, and MHS doesn't replace any of it.
...
So the interesting change is less about automating a motor. It's that a general-purpose agent can orchestrate workflows across different machines that were never individually programmed to work together. That is the job that used to take a specialist weeks per workflow.

The industrial world seems to be moving toward the same junction from its own side. The OPC Foundation is making more than 430 of its companion specifications easier for AI systems to consume through RAG and MCP. Existing industrial infrastructure and AI-facing layers look like they're converging, not competing.
...
That's a much better shape for physical AI than a chatbot with its hand on a valve.
...
Several pilots exposed the same boundary: connecting a machine is easier than understanding its physics.

The reference sheet can answer three questions. What can I read? What can I change? What are the limits? It does not answer two others. Why did this physical process fail? What would an experienced engineer infer from that failure? Those still come from a person who has spent years watching things foam.

## How early this is

It's a research preview. It isn't open source yet. Every published example runs on Claude, though the standard is model-agnostic by design. The results are partner-reported and preliminary, and nobody outside the preview has replicated them. Whether it gets adopted widely is genuinely uncertain. A standard nobody outside the preview can download is still a promise.
...
That knowledge already exists. It's in the operator at a compressor station who knows the third unit runs hot on cold mornings and you wait ten minutes before trusting its reading. It's in the technician who knows a pump must never run dry, and the scientist who knows foam means change the procedure, not retry the command. MHS is the first standard I've seen with a place to write that down. Writing it down is the easy part. Knowing it is still the job.
